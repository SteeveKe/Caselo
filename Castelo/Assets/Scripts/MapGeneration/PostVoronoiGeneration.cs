...
namespace MapGeneration
{
    ...
    public class PostVoronoiGeneration : MonoBehaviour
    {
        ...
        private List<PostGenerationBiome> _postGenerationBiomes;
        private Vector2Int _safeZoneSize;
        private Biome _safeZoneBiome;
        ...
        public void GeneratePostBiome()
        {
            ...
            _safeZoneSize = _mapGeneration.safeZoneSize;
            _safeZoneBiome = _mapGeneration.safeZoneBiome;
            _postGenerationBiomes = _mapGeneration.postGenerationBiomes;
            
            GenerateSafeZone();
            InitBiomePosition();


            foreach (PostGenerationBiome postGeneration in _postGenerationBiomes)
            {
        ...
        private void GenerateSafeZone()
        {
            ...
            for (int i = height / 2 - _safeZoneSize.y / 2; i < height / 2 + _safeZoneSize.y / 2 + _safeZoneSize.y % 2; i++)
            {
                for (int j = width / 2 - _safeZoneSize.x / 2; j < width / 2 + _safeZoneSize.x / 2 + _safeZoneSize.x % 2; j++)
                {
                    _textureBiome[j, i] = _safeZoneBiome;
